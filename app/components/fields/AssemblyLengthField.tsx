...
export default function AssemblyLengthField({ value, onChange }: Props) {
  const adjust = (delta: number) => {
    onChange(Math.max(5, Math.min(120, value + delta)));
  };

  return (
    <div className="space-y-1.5">
      <label className="block text-sm font-semibold text-gray-800">Length (minutes)</label>
      <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (!isNaN(v)) onChange(Math.max(5, Math.min(120, v)));
          }}
