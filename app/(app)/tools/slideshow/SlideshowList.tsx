...
import GenerateModal, { type SlideshowPrefill } from "@/app/components/slideshow/GenerateModal";
...
export default function SlideshowListPage({
  ...
  /** Step-one values from Jo, decoded on the server. See page.tsx. */
  ...
  // Open straight away when Jo sent values through, so the teacher lands on the
  // filled-in wizard rather than on a deck list they then have to act on. This
  // is the slideshow equivalent of a tool form arriving prefilled.
  const [generateOpen, setGenerateOpen] = useState(!!prefill);
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      {generateOpen && (
        <GenerateModal prefill={prefill} onClose={() => setGenerateOpen(false)} />
      )}
    </div>
  );
}
